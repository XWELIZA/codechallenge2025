...
def match_single(
    ...
    has_mutation = (mut_1 | mut_2 | mut_3 | mut_4) & (~has_shared_allele) & (~is_inconclusive)
    ...
    for idx in candidate_indices:
        ...
        for loc_i in range(n_loci):
            ...
            if shared_vec[loc_i]:
                ...
                shared_vals = []
                if abs(q_1 - d_1) < 0.01 or abs(q_1 - d_2) < 0.01: shared_vals.append(q_1)
                if abs(q_2 - d_1) < 0.01 or abs(q_2 - d_2) < 0.01: shared_vals.append(q_2)
                
                if not shared_vals: continue
